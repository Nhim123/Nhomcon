...
export const Nav = ({ isHero = false }) => {
  const [nav, setNav] = useState(false);
  const [dropdown, setDropdown] = useState(false);

  const handleNav = () => {
    setNav((prevNav) => !prevNav); // Sử dụng hàm callback để đảm bảo trạng thái được cập nhật đúng
  };
      ...
      <nav className='fixed top-0 left-0 w-full shadow-md z-50'>
        <div className='grid grid-cols-2 max-container py-4 w-full max-w-[1240px] mx-auto'>
          ...
          <div onClick={handleNav} className='block md:hidden'>
            ...
            <div className={`${nav ? 'fixed left-0 top-0 w-[100%] h-full border-r border-r-gray-900 bg-[#f3ba40]' : 'fixed left-[-100%] transition-all duration-300'}`}>
              <ul className='uppercase p-4'>
                ...
                <li className='p-4 border-b border-gray-600'><a href='/test'>Test</a></li>
              </ul>
            </div>
          </div>
        </div>
